...
function AboutScrollStage() {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const focalX = vpW * A_FOCAL_RATIO; // 텍스트 우측 = Active 카드 위치
      const dx0 = focalX - (cardCx[A_START_I] || 0); // 시작 시 첫 카드를 focal(우측)에
      geo.current = {
        vh,
    ...
    <section ref={sectionRef} aria-label="회사 소개" className="relative bg-white">
      <div ref={trackRef} style={{ height: `${A_TRACK_VH}vh` }} className="relative">
        <div className="sticky top-0 h-screen w-full overflow-hidden">
          ...
          {/* 이미지 슬라이더 — center-focus (top/height JS 제어), 좌우 패딩 0 (full-bleed). 텍스트 뒤(z-0) */}
          <div
            ref={sliderWrapRef}
